import { StyleSheet, Text, View, Image, ImageBackground, TextInput, FlatList, TouchableOpacity, ScrollView } from 'react-native';
...
export default function Appointments({ navigation }) {
    ...
    const dateFilter = () => {
        ...
        const filterDataToday = Data.filter(
          function (item) {
            const itemDate = new Date(item.dateOfArrival);
            const itemTimestamp = itemDate.setHours(0, 0, 0, 0);
            return itemTimestamp === timestamp;
        ...
        const filterDataWeek = Data.filter(
          function (item) {
            const itemDate = new Date(item.dateOfArrival);
            const itemTimestamp = itemDate.setHours(0, 0, 0, 0);
            return itemTimestamp >= timestampWeek && itemTimestamp <= timestampLastDayOfWeek;
          });
        ...
        const filterDataMonth = Data.filter(
          function (item) {
            const itemDate = new Date(item.dateOfArrival);
            const itemTimestamp = itemDate.setHours(0, 0, 0, 0);
            return itemTimestamp >= timestampMonth && itemTimestamp <= timestampLastDayOfMonth;
          });
        ...
        <ImageBackground style={styles.bg} source={require("../assets/img/bg.png")}>
            <View style={styles.container}>
                ...
                <View style={styles.section}>
                    ...
                    {todayData.map((item)=>(
                     <ListItem key={item.id} navigation={navigation} name={item.name} dateOfArrival={item.dateOfArrival} cost={item.cost} disease={item.disease} medication={item.medication} gender={item.gender} dob={item.dob} />
